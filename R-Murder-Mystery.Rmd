...
PS: The police have no good system to collect crime scene reports. Therefore the file structure of the crime scene reports is somewhat different compared to the other data.
...
<p>
## Part 0: Read data
Download and read the data. If you ever make use of the code solutions, the data for those are stored in the relative path of "raw-data/". You can use `here()` to set a relative path.
...
```{r eval=FALSE}
# Unzip data
unzip(here("raw-data/r-murder-mystery.zip"),
      exdir = here("raw-data/"))
...
crime_scene_report_files <- dir_ls(here("raw-data/crime_scene_report/"))
...
```{r eval=FALSE}
# Read person data
person <- vroom(here("raw-data/person.csv"))
...
```{r eval=FALSE}
# Read interview file
interview <- vroom(here("raw-data/interview.csv"), 
                          delim = ",")
...
`r hint(id = 1, text = "You need to find out who visited the gym at the gym at the time Annabel noticed someone.")`
`r hint(id = 2, text = "Annabel also noticed that the last part of the members id was 48Z")`
`r hint(id = 3, text = "You need two datasets")`
`r hint(id = 4, text = "You need the 'get_fit_now_member' and 'get_fit_now_check_in' data sets.")`
`r hint(id = 5, text = "You might be able to combine the data.")`
...
```{r eval=FALSE}
# Read data files
get_fit_now_member <- vroom(here("raw-data/get_fit_now_member.csv"))
get_fit_now_check_in <- vroom(here("raw-data/get_fit_now_check_in.csv"))
...
`r hint(id = 3, text = "Remember that Morty saw a licence plate number that contained 'H42W'")`
...
```{r eval=FALSE}
# Read data files
driver_licence <- vroom(here("raw-data/drivers_license.csv"))
...
dplyr::left_join(x = person, y = driver_licence, by = c("license_id" = "id")) %>%
